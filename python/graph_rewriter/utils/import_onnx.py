...
def _get_conv_pool_pads_attr(attrs):
    if ("auto_pad" in attrs):
        ...
        if (padding == 'SAME_LOWER') or (padding == 'SAME_UPPER'):
            ...
            assert padding == 'NOTSET', "Unrecogonized auto_pad value: {}".format(
                padding)
...
def _batchnorm(op, graph, tensors, initializer):
    ...
    if 'epsilon' in attrs:
        ...
        epsilon = -1
    outputs = graph.batchnorm(inputs[0], inputs[1], inputs[2], inputs[3],
                              inputs[4], epsilon)
    return outputs
...
def _conv2d(op, graph, tensors, initializer):
    ...
    outputs = graph.conv2d(input=inputs[0],
                           weight=inputs[1],
                           strides=strides,
                           padding=pads)
    if len(inputs) > 2:
...
def _maxpool2d(op, graph, tensors, initializer):
    ...
    strides = attrs["strides"]
    pads = _get_conv_pool_pads_attr(attrs)
    outputs = graph.maxpool2d(input=inputs[0],
                              kernels=kernels,
                              strides=strides,
                              padding=pads)
    return outputs
...
def _avgpool2d(op, graph, tensors, initializer):
    ...
    strides = attrs["strides"]
    pads = _get_conv_pool_pads_attr(attrs)
    outputs = graph.avgpool2d(input=inputs[0],
                              kernels=kernels,
                              strides=strides,
                              padding=pads)
    return outputs
...
def _resize_nearest_neighbor(op, graph, tensors, initializer):
    ...
    assert len(
        shape
    ) == 2, "ResizeNeareestNeighbor: new size cannot be statically inferred"
...
def _sum(op, graph, tensors, initializer):
    ...
    assert len(
        inputs
    ) == 2, "TASO assumes Sum takes exactly two inputs. Submit a github issue when you see this."
    outputs = graph.add(inputs[0], inputs[1])
    return outputs
...
input_weight_names['BatchNormalization'] = [
    'input', 'scale', 'bias', 'mean', 'var'
]
input_weight_names['Concat'] = [
