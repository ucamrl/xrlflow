...
from graph_rewriter.agents.base_agent import (_BaseAgent, compute_gae,
                                              action_from_logits,
                                              masked_logits)
from graph_rewriter.agents.models import (MainTwinHeadModel_GAT_global,
                                          SubTwinHeadModel_GAT_global)


class GATHierarchicalPPOAgent(_BaseAgent):
    ...
    def __init__(
        ...
        main_model = MainTwinHeadModel_GAT_global(gat_attn_mlp,
                                                  gat_node_update_mlp,
        ...
        ## sub agent
        sub_model = SubTwinHeadModel_GAT_global(gat_attn_mlp,
                                                gat_node_update_mlp,
                                                message_passing_steps,
                                                gat_global_update_mlp,
                                                policy_feature,
        ...
        main_opt = optax.chain(optax.clip_by_global_norm(global_norm),
                               scale_by_adam(), scale_by_schedule(schedule_fn))
        ...
        sub_param = sub_model.init(k2, state_input["xfers"][i])
    ...
    def act(self, states: dict, explore: bool):
        """
        Main policy act on the states

        Sub policy act on the sub-states

        NOTE: states dict is changed by SIDE-EFFECT
        """
        # get key
        ...
        main_logits, main_vf_values = self.main_state.apply_fn(
            self.main_state.params, states["graph"])
        # invalid masking
        ...
        tuples, masks = _build_sub_state_from_main_action(states, main_action)

        # ========= sub action =========
        sub_logits, sub_vf_values = self.sub_state.apply_fn(
            self.sub_state.params, tuples)
        # invalid masking
    ...
    def update(self, states: list[dict], main_actions: list[jnp.ndarray],
        ...
        for i in range(self.update_round):
            ...
            # NOTE: skip incomplete batch, so shape are constant and function jittable
            idxes = idxes[:steps_per_epoch * self.mini_batch_size]
            idxes = idxes.reshape((steps_per_epoch, self.mini_batch_size))
            ...
            for j, idx in enumerate(idxes):
                self.main_state, main_actor_loss, main_vf_loss, main_entropy, main_approx_kl, main_ratio = _update(
                    ...
                    [main_graph_list[i] for i in idx],
                ...
                self.sub_state, sub_actor_loss, sub_vf_loss, sub_entropy, sub_approx_kl, sub_ratio = _update(
                    ...
                    [sub_graph_list[i] for i in idx],
                    jnp.take(sub_mask, idx, axis=0), self.clip_ratio)

                if j == 0:
                    print(f"[update] round {i}: {self.mini_batch_size}")
                    print(main_ratio)
                    print(sub_ratio)
        ...
        # in practice, maybe not all data within a update round get updated
...
def _update_not_jit(train_state: TrainState, gae, returns, old_actions,
    ...
    def loss(params, fn, gae, returns, old_actions, old_log_probs, old_vf,
             graph, mask):
        new_logits, new_vf = fn(params, graph)
