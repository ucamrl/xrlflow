...
from graph_rewriter.utils.inference import inference, print_available_block, get_complexity

FLAGS = flags.FLAGS
flags.DEFINE_string("path", "Unknown", "path to the onnx model")
flags.DEFINE_string("an", "GAT-hierarchical-ppo-v2-", "")
flags.DEFINE_string(
    "timestamp", None,
    "Timestamp of the checkpoint to evaluate in the format YYYYMMDD-HHMMSS")

flags.DEFINE_integer("num_episodes", 5000, "")
flags.DEFINE_integer("clear_backend_round", 200,
...
flags.DEFINE_integer("episodes_per_batch", 30, "How often will we update?")
...
def main(_):
    ...
    spec = {
        ...
        "num_candidates": FLAGS.max_num_candidates,  # no use
    ...
    for current_episode in tqdm(range(start_episode, FLAGS.num_episodes + 1)):
        ...
        while True:
            ...
            action_history.append((int(main_action), int(sub_action)))
            ...
            if done or timestep > FLAGS.horizon:
                ...
                final_runtime = env.get_cost()  # log
                output_file.write(
                    f"Episode: {current_episode} - episode_reward: {episode_reward:.4f}, runtime: {final_runtime:.4f} - average complexity: {ave_complexity}\n"
                )
                output_file.flush()

                print(
                    f"Episode: {current_episode} - Episode timestep: {timestep} - average complexity {ave_complexity:.1f}"
                )
                ...
                if current_episode % FLAGS.episodes_per_batch == 0:
                    ...
                    print(
                        f"policy loss = {main_actor_loss:.4f} - vf loss = {main_vf_loss:.4f} - entropy {main_entropy:.4f} - KL {main_kl:.4f}"
                    )
                    print(
                        f"sub policy loss = {sub_actor_loss:.4f}, sub vf loss = {sub_vf_loss:.4f} - sub entropy {sub_entropy:.4f} - Sub KL {sub_kl:.4f}"
                    )
