...
flags.DEFINE_string("an", "gn-ppo-v4-", "agent name")
flags.DEFINE_string(
    "timestamp", None,
...
flags.DEFINE_integer("measure_interval", 5,
                     "How often to run end-to-end inference?")
...
def main(_):
    ...
    # env
    env = FlatEnvironment_sparse(num_locations=FLAGS.max_num_candidates,
                                 real_measurements=False,
                                 measure_interval=FLAGS.measure_interval,
        ...
        while True:
            ...
            if done or timestep > FLAGS.horizon:
                ...
                if current_episode % FLAGS.episodes_per_batch == 0:
                    ...
                    # best checkpoints
                    if ave_reward > best_episode_rewards:
                        best_episode_rewards = ave_reward
                        try:
                            agent.save(current_episode,
                                       {"best_episode_rewards": ave_reward})
                        except:
                            pass
                    ...
                    # always regularly save
                    agent.regular_save(current_episode,
                                       {"best_episode_rewards": ave_reward})
