...
def get_padding(kernel, padding):
    return "VALID" if sum(padding) == 0 and sum(kernel) > 2 else "SAME"
...
def conv2d(graph,
    ...
    v = graph.conv2d(input=v,
                     weight=w,
                     strides=stride,
                     padding=get_padding(kernel, padding),
                     activation="RELU")
    return v
...
def pool2d(graph, v, pool_type, kernel=(1, 1), stride=(1, 1), padding=(0, 0)):
    if pool_type == 'global_avg':
        pads = "VALID"
        x = graph.avgpool2d(input=v,
                            kernels=kernel,
                            strides=[1, 1],
                            padding=pads)
    elif pool_type == 'avg':
        pads = get_padding(kernel, padding)
        x = graph.avgpool2d(input=v,
                            kernels=kernel,
                            strides=stride,
                            padding=pads)
...
def inception_e(graph, v):
    ...
    v3x3dbl = conv2d(graph,
                     ...
                     kernel=(3, 3),
                     padding=(1, 1))
    v3x3dbla = conv2d(graph,
                      v3x3dbl,
                      out_channels=384,
                      kernel=(1, 3),
                      padding=(0, 1))
    v3x3dblb = conv2d(graph,
                      v3x3dbl,
                      out_channels=384,
                      kernel=(3, 1),
                      padding=(1, 0))
...
def build_graph_inception_v3(batch_size=1):
    graph = ts.new_graph()
    ...
    v = inception_e(graph, v)
    _ = inception_logits(graph, v)
    return graph


if __name__ == '__main__':
    built_graph = build_graph_inception_v3()
    onnx_model = ts.export_onnx(built_graph)
    onnx.save(onnx_model, 'output-data/graphs/inceptionv3.onnx')
