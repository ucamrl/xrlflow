...
const size_t WORK_SPACE_SIZE = (size_t)1 * 1024 * 1024 * 1024;
...
enum {
  ...
  GUID_INPUT = 10,
  GUID_WEIGHT = 11,
  GUID_FREE = 12, // gh512 to mark as needed to be free!
  GUID_PRESERVED = 19,
};
...
public:
  ...
  // gh512
  float run_memorysafe_print(int num_of_runs);
...
public:
  ...
  // gh512: benchmark variance and mean of num_runs
  float measure_oplist_runtime(const std::vector<OpBase*>& list);
  float measure_oplist_runtime_print(const std::vector<OpBase*>& list, int num_runs);
