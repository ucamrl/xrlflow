...
float Model::measure_oplist_runtime_print(const std::vector<OpBase*>& opBaseList, int num_runs) {
  // gh512 benchmark num_runs influence to end-to-end inference latency
  // warmup
  for (int times = 0; times < num_runs; times++)
    for (int i = 0; i < opBaseList.size(); i++)
      opBaseList[i]->forward();
  ...
  for (int times = 0; times < num_runs; times++) {
    ...
    ret.push_back(milliseconds);
  }

  float sum = std::accumulate(ret.begin(), ret.end(), 0.0);
  float mean = sum / ret.size();
  std::vector<float> diff{};
  for (const float& i: ret) {
          float tmp = (i-mean)*(i-mean);
          diff.push_back(tmp);
  }
  float ssum = std::accumulate(diff.begin(), diff.end(), 0.0);
  float var = ssum / diff.size();
  printf("mean %f - var %f ", mean, var);
  return mean;
}
