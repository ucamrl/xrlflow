...
TensorHandle Graph::new_weight(const Tensor& weight)
{
  ...
  t->op.guid = GUID_FREE;  // gh512 to mark as needed to be free!
  t->op.ptr = NULL;
  t->idx = 0;
...
Graph* Graph::preprocess_weights(void)
{
  ...
  // printf("total const fold runtime: %f\n", cont_folding);
...
float Graph::run_memorysafe(void)
{
  ...
  for (it = inEdges.begin(); it != inEdges.end(); it++) {
    ...
    // free data ptr
    // gh512 to mark as needed to be free!
    for (size_t i = 0; i < opList.size(); ++i) {
    // printf("free data ptr\n");
        Op& op = opList[i];
        if (op.ptr->type == OP_INPUT) {
          cudaFree(op.ptr->inputs[0].data_ptr);
        }
    }
...
float Graph::run_memorysafe_print(int num_of_runs)
{
    std::map<Op, int, OpCompare> todos;
    std::map<Op, std::set<Edge, EdgeCompare>, OpCompare>::const_iterator it;
    std::vector<Op> opList;
  ...
  for (it = inEdges.begin(); it != inEdges.end(); it++) {
    ...
    float result = model->measure_oplist_runtime_print(opBaseList, num_of_runs);
